...
export default function SurveyPage({ clientId }) {
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: C.surface, display: 'grid', placeItems: 'center', fontFamily: BODY, padding: 24 }}>
      ...
      <div style={{ textAlign: 'center', maxWidth: 440 }}>
        <div style={{ width: 72, height: 72, borderRadius: '50%', background: '#E0F3EA', display: 'grid', placeItems: 'center', margin: '0 auto 20px', fontSize: 32 }}>✓</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.surface, fontFamily: BODY, padding: '40px 20px' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Quicksand:wght@400;600;700&family=Archivo:wght@400;500;600;700&display=swap');*{box-sizing:border-box}textarea,input{outline:none}textarea:focus,input:focus{border-color:#73017B!important}`}</style>

      <div style={{ maxWidth: 520, margin: '0 auto' }}>
        {/* Logo / Header */}
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <svg width="40" height="40" viewBox="0 0 48 48" fill="none" style={{ marginBottom: 12 }}>
            <defs><linearGradient id="sg" x1="0" y1="48" x2="48" y2="0"><stop offset="0" stopColor="#52015A"/><stop offset="0.5" stopColor="#A8108F"/><stop offset="1" stopColor="#E40993"/></linearGradient></defs>
          ...
          <div style={{ fontFamily: DISP, fontWeight: 700, fontSize: 13, color: C.primary, letterSpacing: 1, marginBottom: 6 }}>ENCUESTA NPS</div>
        </div>

        {/* Card */}
        <div style={{ background: '#fff', borderRadius: 20, padding: '32px 28px', border: `1px solid ${C.line}`, boxShadow: '0 4px 24px rgba(115,1,123,.08)' }}>
          ...
          <p style={{ color: C.tx2, fontSize: 14, margin: '0 0 28px', lineHeight: 1.5 }}>
            En una escala del 0 al 10, donde 0 es "nada probable" y 10 es "totalmente seguro".
          </p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button
              ...
              style={{
                ...
                background: score === null ? C.line : C.grad,
                color: score === null ? C.tx3 : '#fff',
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: C.tx3, marginTop: 20 }}>
          Powered by <strong style={{ color: C.primary }}>PromotIA</strong> · Tu respuesta es anónima
        </p>
      </div>
    </div>
  )
